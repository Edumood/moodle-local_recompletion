...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table('local_recompletion_ccrt_i', [
                'customcertid' => 'privacy:metadata:local_recompletion_ccrt_i:customcertid',
                'userid' => 'privacy:metadata:userid',
                'code' => 'privacy:metadata:local_recompletion_ccrt_i:code',
                'emailed' => 'privacy:metadata:local_recompletion_ccrt_i:emailed',
                'timecreated' => 'privacy:metadata:local_recompletion_ccrt_i:timecreated',
                'course' => 'privacy:metadata:course',
        ], 'privacy:metadata:local_recompletion_qr');
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            $records = $DB->get_records('local_recompletion_ccrt_i', $params);
            foreach ($records as $record) {
                $context = \context_course::instance($record->course);
                writer::with_context($context)->export_data(
                        [get_string('recompletion', 'local_recompletion'), 'customcert_issues'],
                        (object)[array_map([self::class, 'transform_db_row_to_session_data'], $records)]);
            }
        }
    }
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        $DB->delete_records('local_recompletion_ccrt_i', $params);
    }
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        $sql = "SELECT ctx.id
                  FROM {course} c
                  JOIN {context} ctx ON c.id = ctx.instanceid AND ctx.contextlevel = :contextlevel
                  JOIN {local_recompletion_ccrt_i} rc ON rc.course = c.id and rc.userid = :userid";
        $contextlist->add_from_sql($sql, $params);
        return $contextlist;
    }
    /**
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $sql = "SELECT rc.userid
                  FROM {local_recompletion_ccrt_i} rc
                  JOIN {course} c ON rc.course = c.id
                  JOIN {context} ctx ON c.id = ctx.instanceid AND ctx.contextlevel = :contextlevel
                  WHERE ctx.id = :contextid";
        $userlist->add_from_sql('userid', $sql, $params);
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        $sql = "SELECT rc.id
                  ...
                  WHERE ctx.id = :contextid AND rc.userid $insql";
        $params = array_merge($inparams, ['contextid' => $context->id]);
        $DB->delete_records_select('local_recompletion_ccrt_i', "id $sql", $params);
